...
int main(int argc, char* argv[]) {
	...
	army1.addUnit(avi, 2);
	army2.addUnit(avi, 2);
	army1.addUnit(tank, 5);
	army2.addUnit(tank, 3);
	army1.addUnit(art, 2);	
	army2.addUnit(art, 2);
	army1.addUnit(patr, 1);
	army2.addUnit(patr,1);
	Iterator* it = army1.createSequentiallyTypeIterator();
	while (it->hasMore()) {
		cout << it->getNext()->toString()<<endl;
	}
	delete it;
	Iterator* it2 = army1.createEachTypeIterator();
	while (it2->hasMore()) {
		cout << it2->getNext()->toString() << endl;
	}
	delete it2;
