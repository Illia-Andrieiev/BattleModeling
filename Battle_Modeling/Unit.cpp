...
///Remove first equal item by name 
void Unit::removeItem(const std::string& itemName) {
...
///Add item
void Unit::addItem(const Item& item) {
...
void Unit::setPowerCoef(const std::pair<unitHelpers::unitTypes, double>& powerCoef) {
	this->powerCoef[powerCoef.first] = powerCoef.second;
}
bool Unit::getFortificationTarget() const {
	return fortificationTarget;
}
bool Unit::getIsRenovateArmor() const {
	return isRenovateArmor;
}
double Unit::getMaxArmor() const {
	return maxArmor;
}
unitHelpers::unitTypes Unit::getPriorityTargetType() const {
	return priorityTarget;
}
void Unit::setCycling(const unitHelpers::Cycling& cycling) {
